...
class XPlanListView(FilterView, SingleTableView, LoginRequiredMixin):
    ...
    Klasse für die Anzeige aller Pläne, auf die ein Nutzer Leseberechtigung hat. Ein Nutzer hat Leseberechtigung, wenn er
...
class XPlanListViewHtml(FilterView, ListView):
    """
    Klasse wird für die Rückgabe einer Liste von Pläne bei einer GetFeatureInfo Anfrage verwendet.
    Da die Bereitstellung immer pro Organisation erfolgt, kann man die Organisation als Vorfilter setzen.
    """
    model = BPlan
    model_name_lower = str(model._meta.model_name).lower()
    template_name = 'xplanung_light/' + model_name_lower + '_list_html.html'
...
class XPlanDetailXPlanLightZipView(XPlanDetailView):  
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if self.model_name_lower == 'fplan':
            return FileResponse(zip_buffer, as_attachment=False, filename="flaechennutzungsplan_" + str(self.kwargs['pk']) + ".zip")