...
from xplanung_light.models import BPlan, Uvp
...
"""
class UvpCreateView(CreateView):
    model = Uvp
    form_class = UvpForm
    ...
    def get_success_url(self):
        return reverse_lazy("uvp-list", kwargs={'bplanid': self.kwargs['bplanid']})   
    

class UvpListView(SingleTableView):
    model = Uvp
    table_class = UvpTable
    template_name = 'xplanung_light/Uvp_list.html'
    success_url = reverse_lazy("uvp-list") 
...
class UvpUpdateView(UpdateView):
    model = Uvp
...
class UvpDeleteView(DeleteView):
    model = Uvp
