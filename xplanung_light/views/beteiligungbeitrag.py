...
class BPlanBeteiligungBeitragListView(SingleTableView):
    """
    ListView zur Anzeige der BPlanBeteiligungBeitrag-Records.
...
class BPlanBeteiligungBeitragCreateView(EditCollectionView):
    ...
    def get_context_data(self, **kwargs):
        ...
        beteiligung = None
        try:
            beteiligung = BPlanBeteiligung.objects.get(pk=self.kwargs['pk'], allow_online_beitrag=True)
        except:
            pass
    ...
    def form_collection_valid(self, form_collection):
        ...
        if result:
            ...
            activation_url = reverse("bplanbeteiligungbeitrag-activate", args=[self.object.bplan.id, self.object.id, beitrag_generic_id])
            # Build complete URLs
            #https://opensource.com/article/22/12/django-send-emails-smtp
            #
            activation_link = f"{activation_url}"
            email = EmailMessage(
                subject=str("Ihre Stellungnahme von " + datetime.today().strftime('%Y-%m-%d') + " zum Bebauungsplan " + str(self.object.bplan.name)),
